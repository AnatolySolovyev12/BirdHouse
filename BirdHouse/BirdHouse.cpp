...
{
	if (offChanger)
		return; // препятствуем многократному исполнению этой функции при изменении цветов
...
void BirdHouse::otherItemWasChecked(QTreeWidgetItem* any, int columnFunc) // закрываем открытый редактор в случае переключения на другой элемент
{
	...
	if (middleItem) // Производим запись значения из LineEdit редактора при переходе на другую ячейку
	{
		QModelIndex idx = ui.treeWidget->indexFromItem(middleItem, middleColumn); // Берем ссылку на ячейку 

		if (idx.isValid())
		{
			QWidget* editor = ui.treeWidget->indexWidget(idx); // получаем указатель на физический editor‑виджет (открытый редактор).

			if (editor)
			{
				if (auto* lineEdit = qobject_cast<QLineEdit*>(editor)) // делаем проверку является ли указатель LineEditor. Если нет то там будет nullptr и мы его ен будм разименовывать.
				{
					QString text = lineEdit->text().trimmed(); // берём данные из редактора и пишем их в ячейку
					middleItem->setText(middleColumn, text);
				}
			}
		}
	}
